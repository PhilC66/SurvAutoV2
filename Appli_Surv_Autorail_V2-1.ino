...
struct  config_t 										// Structure configuration sauvée en EEPROM
{
  ...
  boolean Pos_PN 	;									// envoie un SMS au premier lancement apres 30s
  ...
  int 		Dsonnrepos;								// Durée repos Sonnerie
	int 		Jour_TmCptMax;	 					// Jour Temps de la boucle fausses alarme 1200 = 2mn V2-122
	int 		Jour_Nmax ;				        // Jour Nombre de fausses alarmes avant alarme V2-122
  char 		Idchar[11];								// Id
	byte    PirActif[4];							// Capteur PIR actif	
	// V2-122
	boolean IntruAuto;								// pas utilisé Mode Alarme Intrusion automatique entre Hsoir et Hmatin
	long 		IntruFin;									// Heure arret Alarme Intru Matin parametre jour/nuit
	long 		IntruDebut;								// Heure debut Alarme Intru Soir
	int 		Nuit_TmCptMax;						// Nuit Temps de la boucle fausses alarme 1200 = 2mn
...
byte recordn = 75;									// Num enregistrement EEPROM V2-122
...
void setup() {
  ...
  if (config.magic != Magique) {											//V2-14
    ...
    temp.toCharArray(config.Idchar, 11);
		...
		config.Nuit_TmCptMax = 900;       // V2-20 90//60s V2-14
    ...
    int longueur = EEPROM_writeAnything(confign, config);	// ecriture des valeurs par defaut
		...
		Serial.print(F("longEEPROM1=")),Serial.println(longueur); //long=130
...
void loop() {
  ...
  /* test seulement */
	...
	if (config.Intru && (CptAlarme1 > 0 || CptAlarme2 > 0 || CptAlarme3 > 0 || CptAlarme4 > 0 )){	
		//Alarm.delay(500);			// si alarme sur coupure batterie, attendre chute de tension avant mesure
		if (map(moyenneAnalogique(), 0, 1023, 0, config.CoeffTension) > 1150){ // prise en compte seulement si Vbatt OK, fausse alarme si Vbatt coupée	V2-122
			...
			if (timecompte > TmCptMax || FlagPIR){ 	// remise à 0 du comptage apres delai ou alarme detectée		V2-122					
				...
				timecompte = 0;									// on ne divise plus par 2 (filtre hard)
				FausseAlarme1  += CptAlarme1;		//divise par 2 car 1 alarme = 2cpt
				FausseAlarme2  += CptAlarme2;		//divise par 2 car 1 alarme = 2cpt
				FausseAlarme3  += CptAlarme3;		//divise par 2 car 1 alarme = 2cpt
				FausseAlarme4  += CptAlarme4;		//divise par 2 car 1 alarme = 2cpt				
				CptAlarme1  = 0;			
				CptAlarme2  = 0;
				CptAlarme3  = 0;			
				CptAlarme4  = 0;				
...
fin_i:
        ...
        sendSMSReply(callerIDbuffer, sms);
				...
				if (!config.Intru) {
					...
					if(!sms){															//V2-14
						String bidon = F("console");
						bidon.toCharArray(nameIDbuffer,8);	//	si commande locale
					}
				...
				if (textesms.indexOf(char(61))== 7) { //char(61) "="	liste capteur actif
					byte Num[4];
					String bidon=textesms.substring(8,15);
				...
				sendSMSReply(callerIDbuffer, sms);
					...
					if ((textesms.indexOf("?")== 8) || (textesms.indexOf(char(61))== 8)){ //char(61) "="
						...
						//Serial.println("bidon="),Serial.print(bidon),Serial.println(bidon.length());
        ...
        //}
				...
				if (textesms.indexOf(char(61)) == 5){ //"="
					...
					if (i > 0 && i < 101 && j > 9 && j < 601 && 
						...
						config.Jour_Nmax 		 = i;
						config.Jour_TmCptMax = j * 10; //passage en n*100ms
						config.Nuit_Nmax 		 = k;
				...
				message += F(", t = ");				
				message += config.Jour_TmCptMax / 10;
				message += F("(s)");
				message += fl;
				message += F("Nuit n = ");				
				message += config.Nuit_Nmax;
				message += F(", t = ");				
				message += config.Nuit_TmCptMax / 10;
				message += F("(s)");
				message += fl;
				message += F("Actuel n = ");				
				message += Nmax;
...
void generationMessage() {
  ...
  if ( FlagAlarmeTension || FlagLastAlarmeTension || FlagAlarmeIntrusion) {
    ...
    message += F("Alarme Motrice");
		message += fl ;
		message += F("1 = ");
    message += FausseAlarme1;// plus divise 2
		message += F(", 2 = ");
...
void MajHeure(){
  ...
  if (First) {																// premiere fois apres le lancement    
    ...
    if (abs(ecart) > 5){
			...
			Alarm.enable(Svie);
			Alarm.enable(HIntruD);//V2-122
			Alarm.enable(HIntruF);//V2-122
...
//---------------------------------------------------------------------------
void OnceOnly() {
  Serial.print(F("Premier message apres lancement ! "));
  Serial.println(Alarm.getTriggeredAlarmId());
...
void decodeGPS() {
  ...
  byte cpt = 0;
	do {
    i = 0;
		Alarm.delay(1000);								// attendre synchro GPS
    fona.getGPS(0, gpsdata, 120);
    //Serial.println(gpsdata);
    dataGPS = String(gpsdata);
    i = dataGPS.indexOf(",", i + 1);
    fix = dataGPS.substring(i + 1, i + 2).toInt();			
		if (cpt ++ > 2) break;					// 20 sortir si GPS ne synchronise pas		
  ...
  i = dataGPS.indexOf(",", i + 1);
  j = dataGPS.indexOf(",", i + 1);
  speed_kph	= dataGPS.substring(i + 1, j).toFloat();
  i = dataGPS.indexOf(",", i + 1);
  j = dataGPS.indexOf(",", i + 1);
  ...
  Serial.print(F("Vitesse = ")), Serial.print(speed_kph, 1), Serial.print(F(" km/h, dir = ")), Serial.println(heading, 0);
}
//---------------------------------------------------------------------------
...
void AllumeCapteur(){		// allumage des capteurs selon parametres
	...
	// V2-18
	for(int i = 0; i < 10; i++){
		if(config.PirActif[0]){digitalWrite(Op_PIR1, HIGH);}
		if(config.PirActif[1]){digitalWrite(Op_PIR2, HIGH);}
		if(config.PirActif[2]){digitalWrite(Op_PIR3, HIGH);}
		if(config.PirActif[3]){digitalWrite(Op_PIR4, HIGH);}
		Alarm.delay(400);
		if(config.PirActif[0]){digitalWrite(Op_PIR1, LOW);}
		if(config.PirActif[1]){digitalWrite(Op_PIR2, LOW);}
		if(config.PirActif[2]){digitalWrite(Op_PIR3, LOW);}
		if(config.PirActif[3]){digitalWrite(Op_PIR4, LOW);}
		if((i % 5) == 0){
		Alarm.delay(500);
		}
		else{
